...
export default function ShopGrid() {
  ...
  const smallCards = shopGridCards.filter((card) => card.type === "small").slice(0, 4);
  ...
  return (
    <section className="w-full px-4 sm:px-6 py-12 bg-white">
      {/* Container: 
          w-full ensures it tries to fill the screen.
          px-4 sm:px-6 ensures it DOES NOT touch the edges (safe margin).
      */}
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 gap-6 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:grid-rows-2">
          {/* ⭐ LARGE CARD (LEFT) - Premium Gradient */}
        <div className="relative overflow-hidden rounded-2xl p-8 flex flex-col justify-center sm:col-span-2 lg:col-span-2 lg:row-span-2 bg-gradient-to-br from-green-50 to-green-100 shadow-lg min-h-80 sm:min-h-96">
          ...
          <img src={largeCard.image} alt={largeCard.title} className="absolute right-6 bottom-0 w-[45%] md:w-[48%] object-contain translate-y-5 drop-shadow-xl" />
          </div>

          {/* ⭐ SMALL CARDS */}
          {smallCards.map((card, index) => {
            // Premium Checkerboard Colors
            // Index 0 & 3 = Soft Blue, Index 1 & 2 = Soft Gray
...
function SmallCard({ title, subtitle, img, bgClass }) {
  return (
    <div className={`
      ...
      <img src={img} className="w-28 h-28 object-contain ml-auto translate-y-3" alt={title} />
    </div>
  );
}